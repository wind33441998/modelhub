...
const LICENSE_SERVER = process.env.MODELHUB_LICENSE_SERVER || "http://127.0.0.1:3001";
...
function getCachedLicense() {
  try {
    ...
    // Cache valid for 24 hours (86400000 ms) if cached_at exists
    if (data.cached_at && Date.now() - data.cached_at < 86400000 && data.valid) return data;
    // If offline and cache expired, allow 7-day grace from first_cache
    if (data.first_cached && Date.now() - data.first_cached < 7 * 86400000) return data;
    return null;
  } catch (e) { return null; }
}
...
async function checkLicense() {
  ...
  if (result.offline) {
    const cache = getCachedLicense();
    if (cache && cache.valid) return { ok: true, licensed: true, cache: true, offline: true, ...cache };
    // 7-day trial grace from first install
    if (cache) return { ok: true, licensed: true, grace: true, ...cache };
    return { ok: false, licensed: false, error: "Cannot verify license. Check internet or enter valid key." };
  }
