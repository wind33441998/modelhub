...
function handleMessages(a, res) {
  ...
  // 方案 B：含图片的请求改发视觉模型
  if (containsImage(a)) {
    const vt = visionTarget(target.provider);
    if (vt) { debugLog('VISION_ROUTE', 'messages', modelName || '', '->', vt.provider + '/' + vt.upstream); target = vt; }
    else { debugLog('VISION_ROUTE', 'messages: 含图片但未启用视觉路由，已剔除图片以兼容文本模型（配置 MODELHUB_VISION_MODEL 可启用图片理解）'); a = stripImages(a); }
  }
...
function handleChatCompletions(body, res) {
  ...
  if (containsImage(body)) {
    const vt = visionTarget(target.provider);
    if (vt) { debugLog('VISION_ROUTE', 'chat', body.model || '', '->', vt.provider + '/' + vt.upstream); target = vt; }
    else { debugLog('VISION_ROUTE', 'chat: 含图片但未启用视觉路由，已剔除 image_url 以兼容文本模型（配置 MODELHUB_VISION_MODEL 可启用图片理解）'); body = stripImages(body); }
  }
  const provider = config.providers[target.provider];
...
function handleResponses(body, res) {
  ...
  if (containsImage(body)) {
    const vt = visionTarget(target.provider);
    if (vt) { debugLog('VISION_ROUTE', 'responses', body.model || '', '->', vt.provider + '/' + vt.upstream); target = vt; }
    else { debugLog('VISION_ROUTE', 'responses: 含图片但未启用视觉路由，已剔除图片以兼容文本模型（配置 MODELHUB_VISION_MODEL 可启用图片理解）'); body = stripImages(body); }
  }
  const provider = config.providers[target.provider];
  ...
  if (req.method === 'GET' && url === '/api/selftest') {
    ...
    if (req.method === 'POST' && url === '/api/license/activate') {
      ...
      req.on('end', async () => {
        try {
          const b = JSON.parse(body);
          if (b.key) license.saveLicenseKey(b.key);
