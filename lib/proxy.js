...
function sendSSE(res, event, data) {
  _sseSeq++;
  const d = Object.assign({ sequence_number: _sseSeq }, data);
  ...
  const req = https.request(options, (upstream) => {
    ...
    if (!body.stream) {
      ...
      upstream.on('end', () => {
        ...
        } catch (e) { sendErr(500, '上游响应解析失败: ' + e.message); }
        complete('ok', null);
      });
      upstream.on('error', e => { sendErr(500, e.message); complete('error', e.message); });
      return;
    ...
    upstream.on('error', e => { sendSSE(res, 'response.failed', { type: 'response.failed', response: { id: rid, status: 'failed', error: { message: e.message } } }); res.end(); complete('error', e.message); });
  });
  req.on('error', e => { sendErr(500, e.message); complete('error', e.message); });
...
// ---------- Web 界面 & API ----------
// 内容安全策略：仅允许同源资源与必要的外部字体；限制 connect 到本机代理；禁止被嵌套（审计 M2）
const CSP_HEADER = "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data:; connect-src 'self' http://127.0.0.1:4000; frame-ancestors 'none'";
...
// CORS：仅允许本机代理自身页面 / 同机 localhost / file://(Origin:null) 调用，拒绝任意外部来源（审计 I2）
const ALLOWED_ORIGINS = ['http://127.0.0.1:4000', 'http://localhost:4000', 'null'];
function cors(res, req) {
  const origin = req && req.headers && req.headers.origin;
  const allowed = origin && ALLOWED_ORIGINS.indexOf(origin) !== -1;
  res.setHeader('Access-Control-Allow-Origin', allowed ? origin : 'http://127.0.0.1:4000');
...
function createServer() {
  return http.createServer(async (req, res) => {
    ...
    if (req.method === 'POST' && (url === '/v1/chat/completions' || url === '/chat/completions')) {
      let body = '';
      req.on('data', c => body += c);
      req.on('end', () => { try { handleChatCompletions(JSON.parse(body), res); } catch (e) { res.writeHead(400, { 'Content-Type': 'application/json; charset=utf-8' }); res.end(JSON.stringify({ error: { message: e.message, type: 'invalid_request_error' } })); } });
      return;
    }
    if (req.method === 'POST' && (url === '/v1/responses' || url === '/responses')) {
      let body = '';
      req.on('data', c => body += c);
      req.on('end', () => { try { handleResponses(JSON.parse(body), res); } catch (e) { res.writeHead(400, { 'Content-Type': 'application/json; charset=utf-8' }); res.end(JSON.stringify({ error: { message: e.message, type: 'invalid_request_error' } })); } });
      return;
    }
...
function start(port) {
  ...
  // 退出诊断：把退出码 + 最近请求写入 crash.log（独立于无限增长的 stdout），便于定位 exit -1 根因
